...
!---
  ...
  select case( REFERENCE_1D_MODEL )
    ...
    case(REFERENCE_MODEL_BERKELEY)
      call model_1dberkeley_broadcast(myrank)
  ...
  if(ISOTROPIC_3D_MANTLE) then
    ...
    select case( THREE_D_MODEL )
      ...
      case(THREE_D_MODEL_BERKELEY)
        ...
        call model_berkeley_broadcast(myrank)
  ...
  if( ISOTROPIC_3D_MANTLE .and. r_prem > RCMB/R_EARTH .and. .not. suppress_mantle_extension) then
    ...
    select case( THREE_D_MODEL )
      ...
      case(THREE_D_MODEL_BERKELEY )
        ...
        call model_berkeley_shsv(dble(r_used*R_EARTH_KM),theta,phi,dvsh,dvsv,dvph,dvpv,drho,eta_aniso,iregion_code,CRUSTAL)
...
!      endif
    ...
    case (ICRUST_BERKELEY)
      ! general crustmaps
     ! call model_crustmaps(lat,lon,r,vpc,vsc,rhoc,moho,found_crust,elem_in_crust)
     ! yder masson : check for parameter units !!1
