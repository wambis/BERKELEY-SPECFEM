...
! local variables
  ...
  ! Read source parameters (added by <FM> April, 2021)
  call read_value_double_precision(SOURCE_T1,'solver.SOURCE_T1', ierr)
  if (ierr /= 0) stop 'error occurred with parameter:SOURCE_T1'

  call read_value_double_precision(SOURCE_T2,'solver.SOURCE_T2', ierr)
  if (ierr /= 0) stop 'error occurred with parameter:SOURCE_T2'

  call read_value_double_precision(SOURCE_T3,'solver.SOURCE_T3', ierr)
  if (ierr /= 0) stop 'error occurred with parameter:SOURCE_T3'

  call read_value_double_precision(SOURCE_T4,'solver.SOURCE_T4', ierr)
  if (ierr /= 0) stop 'error occurred with parameter:SOURCE_T4'

  call read_value_double_precision(TAU,'solver.TAU', ierr)
  if (ierr /= 0) stop 'error occurred with parameter: TAU'
  ! ------------ end modification -------------
