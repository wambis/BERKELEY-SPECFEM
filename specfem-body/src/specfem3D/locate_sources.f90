  ...
  do isources_already_done = 0, NSOURCES, NSOURCES_SUBSET_MAX
    ...
    do isource_in_this_subset = 1,NSOURCES_SUBSET_current_size
      ...
      if(TOPOGRAPHY) then
        ...
        print*,'source elevation is:',elevation
      endif
      if(ELLIPTICITY) then
        dcost = dcos(theta)
    ...
    if(myrank == 0) then
      ...
      do isource_in_this_subset = 1,NSOURCES_SUBSET_current_size
        ...
        write(IMAIN,*) 'position of the source that will be used:'
        write(IMAIN,*)
        write(IMAIN,*) '             x: ',x_found_source(isource_in_this_subset)
        write(IMAIN,*) '             y: ',y_found_source(isource_in_this_subset)
        write(IMAIN,*) '             z: ',z_found_source(isource_in_this_subset)
        write(IMAIN,*)
